...
bot.command("reset", async (ctx) => {
  sessions.delete(ctx.chat.id);
  await ctx.reply("🔄 Sessão resetada. Use /start");
});
...
async function handleTypebotMessage(ctx, userMessage) {
  ...
  try {
    if (!sessionId) {
      const startRes = await axios.post(`https://typebot.io/api/v1/typebots/${TYPEBOT_ID}`, { isStreamEnabled: true });
      sessionId = startRes.data.sessionId;
      sessions.set(chatId, sessionId);
    }

    const replyRes = await axios.post(`https://typebot.io/api/v1/sessions/${sessionId}`, { message: userMessage });
    const messages = replyRes.data.messages || [];

    for (const msg of messages) {
      if (msg.type === "text") await ctx.reply(msg.content.text || msg.content);
      else if (msg.type === "image") await ctx.replyWithPhoto(msg.content.url);
      else if (msg.type === "audio") await ctx.replyWithAudio(msg.content.url);
      else if (msg.items) {
    ...
    await ctx.reply("❌ Erro. Use /reset");
  }
}
...
bot.start();
console.log("🚀 Bot Telegram + Typebot online!");
